...
export class FirestoreClient {
  ...
  start(persistenceFactory: PersistenceFactory): Promise<void> {
    ...
    this.credentials.setChangeListener(async user => {
      if (!initialized) {
        initialized = true;
        ...
        await this.initializeRest(user, garbageCollector);
        initializationDone.resolve(); // plumb error
      } else {
